...
int main()
{
    ...
    // so would call real GL functions (via RenderCommand::Init()) even when
    // Application is constructed with the headless NullWindow factory used
    // by Tests/ (see Tests/src/TestSupport/NullWindow.h), which never loads
    // GL function pointers at all - that would crash instead of skip.
    // Only an app that actually intends to render calls this, and only
    // after its window (and GL context) already exist.
